...
function generate(paths) {
    ...
    var componentData = getDirectories(paths.components).map(function(componentName) {
        try {
            ...
            error.push('An error occurred while attempting to generate netadata for ' + componentName + '. ' + error)
        }
    });
    writeFile(paths.output, 'module.expots = ' + JSON.stringify(errors.length ? errors : componentData));
}
...
function getExampleFiles(examplePath, componentName) {
    ...
    try {
        ...
        console.log(`No examples found for ${componentName}.`);
...
function writeFile(filepath, content) {
    fs.writeFile(filepath, content, function(err) {
        err ? console.log(err) : console.log('Component data saved.'); //chalk.red(err)) : console.log(chalk.green('Component data saved.'));
    });
}
